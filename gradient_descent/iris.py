...
filepath = '../data/iris.data.shuf.three'
db = pandas.read_csv(filepath, header=None)
...
# use of variable learning rate has positive results
for i in range(len(Xtrain)):
    output, hot_one =forwardPass(Xtrain.loc[i], weights)
    gradientDescent(Xtrain.loc[i], weights, output, Ytrain[i], LR= 2 * ((len(Xtrain) - i ) / len(Xtrain)))
